    ...
    for b in buku:
        if b["id"] == id_buku:
            if b.get("jumlah", 0) >= qty:
                ...
                # Menyimpan jumlah yang dipinjam 
                peminjaman.append({
                    "nama": nama,
                    "judul": b["judul"],
                    "id": b["id"],
...
def kembalikan_buku():
    ...
    # Daftar peminjaman yang cocok (by nama + judul)
    for p in list(peminjaman):  
        if p["nama"].lower() == nama.lower() and p["judul"].lower() == judul.lower():
            ...
            if days_late > 0:
                ...
                print("Silahkan membayar denda di resepsionis perpustakaan.")
            print()
            return
    ...
    if not peminjaman:
        ...
        for p in peminjaman:
            ...
            try:
                due_dt = datetime.datetime.strptime(p.get("tgl_jatuh_tempo"), "%Y-%m-%d")
                overdue = now > due_dt
                status = "Telat" if overdue else "Masih Dalam Waktu"
            except Exception:
                status = "Unknown"
